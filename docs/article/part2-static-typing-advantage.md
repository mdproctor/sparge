...
**A practical asymmetry: what LLMs generate by default.** The comparison between disciplined Java and disciplined Python is the correct theoretical comparison. In practice, an important asymmetry operates. In Java, type discipline is the default — the compiler refuses untyped code, and deviating from type safety requires deliberate effort. In Python, Pydantic, FastAPI, and pyright strict are opt-in — active choices that must be made, configured, and enforced. Crucially, LLMs themselves do not currently default to generating Pydantic-typed Python. When asked to implement a Python service, an LLM will produce plain dicts, untyped function signatures, and loosely annotated code unless the human architect explicitly instructs otherwise. This is the same training data bias operating one level down: Python's LLM training corpus is predominantly untyped, so LLM-generated Python is predominantly untyped. In practice, reaching the "disciplined Python" comparison point requires the human architect to: choose Pydantic, include it in the project, prompt consistently for typed patterns, and review LLM output to catch slippage back to untyped code. Java requires none of this — the compiler enforces discipline structurally. The best-case comparison remains valid as a theoretical ceiling; the default-state comparison is not symmetric.
...
The largest enterprise installed base — by a substantial margin — means existing Java infrastructure, tooling, hiring pipelines, and institutional knowledge are already in place for most large organisations. This is explicitly a human-team-context argument, not a technical one — and the series owes transparency about invoking it here when Part 1 criticised equivalent arguments for Python.

The distinction is this: Part 1 argued that human familiarity with Python is an *insufficient* reason to choose Python when the technical merits of statically typed languages for LLM-first development are substantially superior. The human factors are real; they do not outweigh the technical ones. Here, those same human factors are cited *in addition to* the technical merits of Java — not as an override of technical analysis. They compound an already-supported recommendation rather than substituting for one.

The symmetrical acknowledgement this requires: Python's human familiarity is also a real factor, and in contexts where a team has deep Python expertise and is using modern typed Python (Pydantic, FastAPI, pyright strict), the human-context case for Python is genuine. The series does not argue that human factors are irrelevant — it argues that for LLM-first application development, they are insufficient on their own and should not override the technical analysis. Enterprise teams with Java expertise have both the technical and the human-context arguments pointing the same direction. Enterprise teams with Python expertise have the human-context argument pointing toward Python and the technical argument — for LLM-first development specifically — pointing toward statically typed languages.
