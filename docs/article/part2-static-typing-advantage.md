...
**The type hint caveat, stated fairly.** Python's optional type annotation system — enforced by tools such as `mypy` and `pyright` — narrows this gap for codebases where annotations are present and maintained. A fully annotated Python codebase with strict type checking applied consistently provides substantially more structural information than unannotated Python. The gap with statically typed languages is real but smaller. The honest qualification is that full annotation is not the norm. It requires discipline to establish and maintain; it is not enforced by the language; and it is unevenly applied in practice, particularly across the boundaries of dependency libraries where the LLM has no control over annotation quality. Static typing is always present, always accurate, and always verified. Python's type hints are sometimes present, sometimes accurate, and never verified by the runtime. *(See Appendix 1 for the related refactoring completeness argument.)*
