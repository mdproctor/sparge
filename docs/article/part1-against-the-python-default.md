...
This connects to the Debugging Decay Index finding cited in Section 2: LLM debugging effectiveness degrades by 60 to 80 percent within two to three iterations. A qualification is needed here: Java compile-fix sequences can also iterate — fixing one type error may expose another, particularly with complex generic type hierarchies or missing checked exception declarations. The series does not claim Java errors are always resolved in one step.

The distinction is narrower but still real: the precision of the error signal determines how much of the LLM's available debugging capacity is consumed by *diagnosis* versus *resolution*. A compiler error specifies the file, line, what was expected, and what was provided — diagnosis is minimal. A runtime error often describes a symptom whose cause requires several reasoning steps to trace — diagnosis is the primary work. Even in a Java compile sequence that requires multiple iterations, each iteration begins with a precise starting point rather than requiring diagnostic reconstruction. The Debugging Decay Index likely applies to any iterative correction sequence; the claim is that Java's error signal quality limits diagnostic consumption per cycle, not that compile sequences never iterate.

---

### A5 — Training Data Distribution and Structural Simplicity: Two Explanations for the Accuracy Gap
